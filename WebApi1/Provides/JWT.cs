...
namespace WebApi3.Providers
{
    ...
    public static class JsonWebToken
    {
        ...
        public static string Encode(IDictionary<string, object> extraHeaders, object payload, byte[] key, JwtHashAlgorithm algorithm)
        {
            ...
            var ePayload = AppHelper.EncryptAES(JsonConvert.SerializeObject(payload), "EOyABeEAhKgZl8nga274jeEBqScmNmGb/AG1bmv3aOA=", "hQBAFjvbEnr3YzIQ1uMCmw==");

            byte[] headerBytes = Encoding.UTF8.GetBytes(JsonConvert.SerializeObject(header));
            byte[] payloadBytes = Encoding.UTF8.GetBytes(ePayload);
            ...
            if (stringToSign == "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.aHRUUHczaGh4aTFpbTJzLzVuTXU5QT09")
            {
                var a = "";
            }
            ...
            var aaa = Convert.ToBase64String(signature);
        ...
        public static string Decode(string token, byte[] key, bool verify = true)
        {
            ...
            if (verify)
            {
                var content = string.Concat(header, ".", payload);
